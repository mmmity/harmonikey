...
После того, как текст закончится, результаты запуска (имя пользователя, файл с тренировкой, игровой режим (пока что захардкоженные), количество слов, количество символов, затраченное время, количество ошибок) сохранится в `stats/stats.csv` тоже в корневой папке проекта.
