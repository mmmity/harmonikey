...
Скачать ассеты: `PYTHONPATH=src python load_assets.py`, находясь в корне проекта (чтобы запускать все из папки src).
Запустить: `PYTHONPATH=src python main.py`, находясь в корне проекта (чтобы запускать все из папки src)
...
Проект можно установить из PyPI. Для этого применить команду ` pip install --extra-index-url https://test.pypi.org/simple/ harmonikey-mmmity==0.0.1`.
После этого, для запуска проекта достаточно написать `harmonikey` в том же окружении, а для загрузки ассетов - `harmonikey-load-assets`
